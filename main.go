...
import (
	...
	"github.com/shareed2k/pjgo/pjsua2"
	"os"
	"strings"
)
...
func main() {
	...
	//call := make(chan pjsua2.Call)
	createCall := make(chan string)
	quit := make(chan int)

	calls := make(map[int]pjsua2.Call, 0)
	for {
		select {
		case <-createCall:
			op := pjsua2.NewCallOpParam(true)

			call := pjsua2.NewCall(acc)
			call.MakeCall("sip:100@192.168.5.48", op)
